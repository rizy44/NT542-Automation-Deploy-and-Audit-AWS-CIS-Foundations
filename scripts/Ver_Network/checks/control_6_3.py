"""
CIS 6.3: Ensure no security groups allow ingress from 0.0.0.0/0 to SSH/RDP (Automated)
"""
import logging
...
def has_unrestricted_ssh_rdp_ipv4(security_group):
    """Check if SG allows SSH/RDP from 0.0.0.0/0"""
...
@error_handler
def check_control_6_3(profile_name=None, regions=None, **kwargs):
    control_id = "6.3"
    ...
    for region in regions:
        try:
            ...
            for sg in response.get("SecurityGroups", []):
                is_compliant = not has_unrestricted_ssh_rdp_ipv4(sg)
    ...
    return create_result(
        ...
        remediation="Remove security group rules allowing SSH/RDP from 0.0.0.0/0" if status == "FAIL" else None,
    )
