"""
CIS 4.8: Ensure object-level logging for write events is enabled for S3 buckets.
"""
import logging
...
@error_handler
def check_control_4_8(profile_name=None, regions=None):
    control_id = "4.8"
    ...
    if not trails:
        return create_result(
            ...
            remediation="Configure CloudTrail S3 data events for write activity",
        )
    ...
    for trail in trails:
        ...
        try:
            ...
            has_logging = event_selectors_have_s3_logging(response, "write")
            all_details.append(
                {
                    "trail_name": trail_name,
                    "trail_arn": trail.get("TrailARN"),
                    "has_s3_write_logging": has_logging,
    ...
    return create_result(
        ...
        control["severity"],
        details={
            "trails_with_write_logging": enabled,
            "trails_without_write_logging": disabled,
            "all_trails": all_details,
            "discovery_errors": discovery_errors,
        },
        resource_id=",".join(enabled) if enabled else None,
        remediation="Enable CloudTrail S3 object-level data event logging for write events" if status == "FAIL" else None,
    )
